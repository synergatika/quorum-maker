...
console.log(colors.white('Value 50 has been set!\n'));
 console.log(colors.yellow('Public transaction hash result of value being set is ')+ result.tx);
 storage.get().then(function(result){
    console.log(colors.yellow('Value being retrieved by a public transaction is ') + result);
    done();
}).catch(done);
