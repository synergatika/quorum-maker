...
curl -s -X POST --data '{"jsonrpc":"2.0","method":"eth_accounts","params":[],"id":83}' ${ip}:${rpcport} > resultAccts.json
...
curl -s -X POST --data '{"jsonrpc":"2.0","method":"personal_unlockAccount","params":['"$address"', "", 0],"id":1}' ${ip}:${rpcport} > unlock.json
